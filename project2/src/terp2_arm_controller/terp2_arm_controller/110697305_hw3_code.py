...
def get_sympy_inputs(theta_1=0, theta_2=0, theta_3=0, theta_4=0, theta_5=0, theta_6=0):
    ...
    a     = [        0,         .73731,   .3878,               0,         0,       0  ] # Meters
    alpha = [ -np.pi/2,              0,       0,        -np.pi/2,  -np.pi/2,        0 ] # Radians
    d     = [    .1833,              0,       0,          -.0955,     .1155,   .0768 ] # Meters
...
def get_lamdified(J, T01,  T02, T03, T04, T05, T06, start_values_fixed, theta_variables):
    ''' This function takes in the symbolic transformation matrices and Jacobian, and returns lambdified functions'''
    ...
    func_mats = { 'J_func': J_func,    'T01_func': T01_func, 
                 'T02_func': T02_func, 'T03_func': T03_func,
                 'T04_func': T04_func, 'T05_func': T05_func, 'T06_func': T06_func}
    
    subs_mats = { 'J_subs': J_subs,    'T01_subs': T01_subs,
                    'T02_subs': T02_subs, 'T03_subs': T03_subs,
                    'T04_subs': T04_subs, 'T05_subs': T05_subs, 'T06_subs': T06_subs}
...
func_mats, subs_mats = get_lamdified(J, T01,  T02, T03, T04, T05, T06, start_values_fixed, theta_variables)
J_func, T06_func   = func_mats['J_func'], func_mats['T06_func']
...
theta_4, theta_5, theta_6           =  -np.pi/2, 0, 0
start_values, theta_variables       = get_sympy_inputs(theta_1=theta_1, theta_2=theta_2, theta_3=theta_3, theta_4=theta_4, theta_5=theta_5, theta_6=theta_6)
keys_to_remove                      = theta_variables
start_values_fixed                  = {key: start_values[key] for key in start_values if key not in keys_to_remove}
theta_current                       = get_sympy_thetas(theta_1=theta_1, theta_2=theta_2, theta_3=theta_3, theta_4=theta_4, theta_5=theta_5, theta_6=theta_6)
home_origins, case_origins = run_test_case(theta_1=theta_1, theta_2=theta_2, theta_3=theta_3, theta_4=theta_4, theta_5=theta_5, theta_6=theta_6, T_home_matrices=T_home_matrices, case_title='test', print_origins=False)


r_1   = sp.Matrix([0,           0,    -.015, 1 ]) # Frame 1 Coordinates
r_2   = sp.Matrix([ -2/3*.7371,  0 ,   -.1723, 1]) # Frame 2 Coordinates
r_3   = sp.Matrix([  -.2439,      0,       0, 1]) # Frame 3 Coordinates
...
T_mats_CoM = [T01, T02, T03, T04, T05, T06]
r_com      = [r_1, r_2, r_3, r_4, r_5, r_6]

p_com = [T_mats_CoM[i] * r_com[i]  for i in range(6)]

o0              = sp.Matrix([0, 0, 0, 1])


# Origins of each CoM
com_origins_sym = p_com

# Z vectors of each joint
Z0        = sp.Matrix([0, 0, 1])
Z_vectors = [T[:3, 2] for T in T_mats_CoM]
...
for i in range(6):
    Z_i_1   = Z_vectors[i]
    o_i_1   = com_origins_sym[i][0:3, 0]
    L       = com_origins_sym[5][0:3, 0] - o_i_1
    J_i_0   = Z_i_1.cross(L)
    J_i     = J_i_0.col_join(Z_i_1)
    J.append(J_i)
J  = sp.Matrix.hstack(*J)
for i in range(6):
    sp.pprint(com_origins_sym[i].subs(theta_current).subs(start_values_fixed))

J_com = J.subs(start_values_fixed).evalf()  
...
for i in range(6):
    ...
    F_i = F[i]
    G_i = J_i.T * F[i]
    G.append(G_i.subs(theta_current).evalf())
...
K = 0
inertial_mats = get_intertial_matrices()

# Symbolic joint velocities
theta_dot_variables = sp.Matrix(sp.symbols('theta_dot_1:7'))
theta_ddot_variables = sp.Matrix(sp.symbols('theta_ddot_1:7'))

# Symbolic joint velocities and accelerations
q      = sp.Matrix(theta_variables)
q_dot  = sp.Matrix(theta_dot_variables)
q_ddot = sp.Matrix(theta_ddot_variables)
...
D = get_D_of_q(inertial_mats, J, theta_current, T_i_com, m)



# %% Run Custom Case
theta_1_start, theta_2_start, theta_3_start, theta_4_start,  theta_5_start, theta_6_start = 0, -np.pi/2, 0, 0, 0, 0 
...
# theta_1_start, theta_2_start, theta_3_start, theta_4_start, theta_5_start, theta_6_start =0, -np.pi/2, 0, -np.pi/2, 0, 0 
...
linear_phase_velocity = 25 / 1000      # m/s
