...
terp2_controller::terp2_controller() : Node("terp2_controller") {
...
void terp2_controller::model_state_callback(const gazebo_msgs::msg::ModelStates::SharedPtr msg) {
    ...
    if (robot_index > -1) {
        ...
        log("ERROR: terp2 robot not found!");
    }
}

void terp2_controller::update() {
...
void terp2_controller::set_robot_drive_wheels(double velocity) {
    std_msgs::msg::Float64MultiArray message = std_msgs::msg::Float64MultiArray();
    message.data = {velocity, velocity};
    m_pub_v->publish(message);
}

void terp2_controller::set_robot_steering(double steer_angle) {
    double steer_ratio = steer_angle / m_steer_max;
    std_msgs::msg::Float64MultiArray message = std_msgs::msg::Float64MultiArray();
    message.data = {-1 * steer_ratio, steer_ratio};
...
void terp2_controller::set_rotational_goal() {
...
void terp2_controller::set_distance_goal() {
...
double terp2_controller::distance_formula(const double x1, const double y1, const double x2, const double y2) {
    double dx = x2 - x1;
    double dy = y2 - y1;
...
void terp2_controller::log(const std::string &msg) const {
    RCLCPP_INFO(this->get_logger(), "%s", msg.c_str());
}

void terp2_controller::log_position() const {
...
void terp2_controller::log_velocity() const {
...
void terp2_controller::log_double(const std::string &msg, double value) const {
    RCLCPP_INFO(this->get_logger(), "%s: %.2f", msg.c_str(), value);
}

int main(int argc, char *argv[]) {
    rclcpp::init(argc, argv);
    rclcpp::spin(std::make_shared<terp2_controller>());
    rclcpp::shutdown();
    return 0;
}
